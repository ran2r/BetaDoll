...
-(void)engine
{
    ...
    //show basic-eye (now eye is the whole face)
     
	eyeSprite = [CCSprite spriteWithFile:[NSString stringWithFormat:@"%@.png",basic_pic]];
	eyeSprite.position=ccp(240,160); //160 175
	[self addChild:eyeSprite];
    
    //show basic-mouth
    //basic_pic= @"mouthBasic";
	//mouthSprite = [CCSprite spriteWithFile:[NSString stringWithFormat:@"%@.png",basic_pic]];
	//mouthSprite.position=ccp(240,160); //160 175
...
-(void)twoSec
{
    ...
    //array count from index 0 and DB col starts from index1
	if( WeInAction==0 && ![parentMode isEqualToString:@"Sleep"])
	{
        
        [self animation:@"Raindeer_Eye_blinking"];
        //[self animation:@"mouthSheet"];
...
-(void)animation:(NSString *)animation
{
    ...
    //remove the specific part befor animation starts, if only eyes are running so basic mouth will not be removed
    
    if( [animation isEqualToString:@"eyeSheet"])
        [self removeChild:eyeSprite cleanup:YES]; 
    
    
    
    if( [animation isEqualToString:@"mouthSheet"])
        [self removeChild:mouthSprite cleanup:YES]; 
	...
	id call=[CCCallFuncND actionWithTarget:self selector:@selector(finishAnimation:data:) data:animation]; //send relevant animation
